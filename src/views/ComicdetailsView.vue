...
export default {
  ...
  methods: {
    ...
    /* 获取漫画详情方法 */
    getComicDetailFun() {
      this.comicId = this.$store.state.comicId;
      ...
      getComicDetail({ comicId: this.comicId }).then((data) => {
        ...
        this.getepListFun(data.data.ep_list.reverse())
    ...
    chooseThisEp(item, index) {
      ...
      //vuex传值
      this.getIndexFun(index);
      this.getepListFun(this.comicDetails.ep_list);
      this.$router.push({
        name: "comicview",
        query: {
          epId: item.epId,
    ...
    async ToComicDetails(id) {
      ...
      await getComicDetail({ comicId: id }).then((data) => {
        ...
        this.getComicIdFun(id);
        this.getIndexFun(index);
        this.getepListFun(ep_list);
      });
      this.$router.push({
        name: "comicview",
